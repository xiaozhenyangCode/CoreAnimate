//
//  ReduceNumberLayersViewController.swift
//  CoreAnimate
//
...
import UIKit
/**
   减少图层数量 Reduce number of layers
...
class ReduceNumberLayersViewController: BaseViewController, UIScrollViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        transform.m34 = CGFloat(-1 / CAMERA_DISTANCE)
        ...
        for z in 1 ... DEPTH {
            for y in 0 ... HEIGHT {
                for x in 0 ... WIDTH {
                    ...
                    layer.backgroundColor = arcrandomColor().cgColor
                    scrollView.layer.addSublayer(layer)
                }
            }
        }
        printf("displayed " + "\(DEPTH * HEIGHT * WIDTH)")
    }
...
//        updateLayers()
    ...
    func updateLayers() {
        ...
        for z in 1 ... DEPTH {
            var adjusted = bounds
            adjusted.size.width = CGFloat(PERSPECTIVE(z * SPACING))
            adjusted.size.height = CGFloat(PERSPECTIVE(z * SPACING))
            adjusted.origin.x -= (adjusted.width - bounds.width) / 2
            adjusted.origin.y -= (adjusted.height - bounds.height) / 2
            for y in 0 ... HEIGHT {
                let a = CGFloat(y * SPACING)
                if (a < adjusted.origin.y) || (a >= (adjusted.origin.y + adjusted.size.height)) {
                    continue
                }
                for x in 0 ... WIDTH {
                    let b = CGFloat(x * SPACING)
                    if (b < adjusted.origin.x) || (b >= (adjusted.origin.y + adjusted.size.width)) {
                        continue
                    }
                    let layer = CALayer()
                    layer.frame = CGRect(x: 0, y: 0, width: SIZE, height: SIZE)
        ...
        printf("displayed " + "\(DEPTH * HEIGHT * WIDTH)  " + "\(visibleLayers.count)")
    }

    func PERSPECTIVE(_ z: Int) -> Int {
