 ...
 */
class TimeFrameViewController: BaseViewController {
...
//    var timer: Timer?
    ...
    func animate() {
        ...
        duration = 1
        timeOffset = 0
        fromValue = NSValue(cgPoint: CGPoint(x: view.center.x, y: 170))
        toValue = NSValue(cgPoint: CGPoint(x: view.center.x, y: view.frame.maxY - 50))
        lastStep = CACurrentMediaTime()
        timer = CADisplayLink(target: Proxy(self), selector: #selector(step))
    ...
    @objc func step() {
        ...
        time = CFTimeInterval(bounceEaseOut(Float(time)))

        let position = interpolateFromValue(fromValue, toValue, Float(time))

        ballImageView.center = position.cgPointValue
